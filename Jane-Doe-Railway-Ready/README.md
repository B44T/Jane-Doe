...
Slash commands are synced directly to Discord at startup and after gateway reconnects so updated options appear correctly. When connected to one server, the bot automatically uses one immediate server-scoped command set even if `GUILD_ID` is blank; stale global registrations are removed to prevent duplicate or outdated commands. `GUILD_ID` can still explicitly select the server. `/announce` supports an `anonymous` option that omits the publisher's name and avatar; automatic announcement cards have the same anonymous toggle in the dashboard. Discord.py's gateway cache keeps custom emojis current without restarting or repeatedly fetching them from Discord; the dashboard checks the local cache once per minute and also offers manual refresh.
